...
int read_key(TeeSecOC* secoc_obj, const char* alias){
    ...
    uint32_t origin;
    res = TEEC_InvokeCommand(&(secoc_obj->session), TA_READ_AES_KEY, &op, &origin);
    if(res != TEEC_SUCCESS){
        printf("read_key : TEEC_InvokeCommand failed 0x%x origin 0x%x\n", res, origin);
        return -1;
    }
...
int gen_aes_mac(TeeSecOC* secoc_obj, TEEC_SharedMemory* in, const char* alias, uint8_t aes_cmac[SECOC_MAC_SIZE]){
    ...
    op.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_INPUT, TEEC_MEMREF_WHOLE, TEEC_MEMREF_TEMP_OUTPUT, TEEC_NONE);
    op.params[0].tmpref.buffer = (void*)alias;
    op.params[0].tmpref.size = strlen(alias);
