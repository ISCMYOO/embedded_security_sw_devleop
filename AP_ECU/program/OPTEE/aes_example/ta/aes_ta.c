...
bool encrypt(TEE_Param params[4], TEE_OperationHandle opHandle)
{
    ...
    uint32_t aadSize = eParams->aadSize;
	uint32_t plainSize = eParams->plainSize;

	unsigned char *aad = eParams->buffer;
	unsigned char *plain = eParams->buffer + aadSize;
	...
	res = TEE_AEInit(opHandle, iv, 12, 128, aadSize, plainSize);
	if (res != TEE_SUCCESS)
	{
...
bool decrypt(TEE_Param params[4], TEE_OperationHandle opHandle)
{
    ...
    uint32_t aadSize = dParams->aadSize;
	uint32_t cipherSize = dParams->cipherSize;
    uint32_t ivSize = dParams->ivSize;
    uint32_t tagSize = dParams->tagSize;

    unsigned char* aad = dParams->buffer;
	unsigned char* cipher = dParams->buffer + aadSize;
    unsigned char* iv = dParams->buffer + aadSize + cipherSize;
    unsigned char* tag = dParams->buffer + aadSize + cipherSize + ivSize;

	if (ivSize != 12 || tagSize != 16) {
		return false;
	}
    res = TEE_AEInit(opHandle, iv, ivSize, 128, aadSize, cipherSize);
    if (res != TEE_SUCCESS)
    {
	...
	void *local_tag = TEE_Malloc(tagSize, 0);
	TEE_MemMove(local_tag, tag, tagSize);
